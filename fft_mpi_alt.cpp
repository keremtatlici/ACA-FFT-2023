/*
In this program, the master does not call fft1d() or ifft1d(). 
All those calculations are done in slave only.
*/
#include <stdio.h>
#include <fstream>
#include <valarray>
...
int main (int argc, char *argv[]) 
{
  ...
  if (myrank == 0)
  {
    ...
    {
      int retVal = MPI_Send(buf[(i - 1)*chunk], MAX * chunk, MPI_C_DOUBLE_COMPLEX, i, 555, MPI_COMM_WORLD);
    }
    ...
    for (int i = 1; i < size; ++i) 
    {
        int retVal = MPI_Recv(buf[(i - 1)*chunk], MAX * chunk, MPI_C_DOUBLE_COMPLEX, i, 555, MPI_COMM_WORLD, &status);
    }
